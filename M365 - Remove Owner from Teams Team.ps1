...
# Type: New Service
#
# Title: M365 - Remove Owner from Microsoft Teams Team
#
# v 1.0 Initial Release
# v 1.1 Added Stored Credentials
#       see for details: https://au2mator.com/documentation/powershell-credentials/?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveOwnerFromTeams&utm_content=PS1
...
# Code Template V 1.3
#
# URL: https://au2mator.com/remove-owner-from-microsoft-teams-team-microsoft-teams-self-service-with-au2mator/?utm_source=github&utm_medium=social&utm_campaign=M365_RemoveOwnerFromTeams&utm_content=PS1
# Github: https://github.com/au2mator/TEAMS-Remove-Owner-from-Teams-Team
#################
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\M365 - Remove Owner from Teams Team"
[string]$LogfileName = "Remove Owner from Teams Team"
...
try {
    ...
    try {
        ...
        #$c_TeamID="1f001975-b067-40e5-9fcc-e730d9f39b36"
        #$c_TeamOwners="Jasmine.Hofmeister@au2mator.com"
        Write-au2matorLog -Type INFO -Text "Try to get User with UPN"
            $TempOwner = $c_TeamOwners
            $URL = "https://graph.microsoft.com/v1.0/teams/$c_TeamID/members"  
            $Result = Invoke-RestMethod -Headers $headers -Uri $URL -Method GET -Verbose
            
            $MembershipID = ($Result.value | Where-Object -Property email -Value "$TempOwner" -eq).ID
        Write-au2matorLog -Type INFO -Text "Got the User ID: $MembershipID"
        

        try {
            Write-au2matorLog -Type INFO -Text "Try to Update Memebership"

            if ($c_KeepAsMember -eq "True")
            {
                Write-au2matorLog -Type INFO -Text "Keep User as Member"
...
"@
                ...
                $URLUpdate = "https://graph.microsoft.com/v1.0/teams/$c_TeamID/members/$MembershipID"  
            ...
            {
                Write-au2matorLog -Type INFO -Text "Remove User from Team" 
                

                $URLRemove = "https://graph.microsoft.com/v1.0/teams/$c_TeamID/members/$MembershipID"  
                Invoke-RestMethod -Headers $headers -Uri $URLRemove -Method DELETE -Verbose
            ...
            $au2matorReturn = "The User $TempMember was added to Teams $($Team.displayname) as $c_TeamRole"
            $AdditionalHTML = "The User $TempMember was added to Teams $($Team.displayname) as $c_TeamRole
            <br>
            "
            $Status = "COMPLETED"
        }
            ...
            Write-au2matorLog -Type ERROR -Text "Could not add Member"
            Write-au2matorLog -Type ERROR -Text "Error: $Error"

            $au2matorReturn = "Could not add Member, Error: $Error"
            $AdditionalHTML = "Could not add Member
            <br>
            Error: $Error
            "
            $Status = "ERROR"
