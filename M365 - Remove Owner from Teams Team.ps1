...
# New Service
# M365 - Remove Owner from Microsoft Teams Team
# v 1.0 Initial Release
# Init Release: 18.05.2020
# Last Update: 18.05.2020
# Code Template V 1.1
# URL: https://au2mator.com/remove-owner-from-microsoft-teams-team-microsoft-teams-self-service-with-au2mator/
...
try {
    ...
    if ($Team) {
        ...
        try {
            ...
            $SplitMembers = $c_TeamOwners.split(";")

            foreach ($Member in $SplitMembers) {
                if ($c_KeepAsMember -eq "True") {
                    Write-au2matorLog -Type INFO -Text "Remove Owner but keep as Member: $Member"
                    $Team | Remove-TeamUser -User $Member -Role Owner
                }
                else {
                    Write-au2matorLog -Type INFO -Text "Remove Owner: $Member"
